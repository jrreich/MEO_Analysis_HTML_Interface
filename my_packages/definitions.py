...
# country = open(os.path.join(os.path.dirname(__file__), "countries.csv"))
...
country_dict = {
    ...
    "730": "Colombia",
    "616": "Comoros",
    "615": "Congo (People's Republic of), ",
    "676": "Congo ( Democratic Republic of)",
    "518": "Cook Islands",
    "321": "Costa Rica",
    "238": "Croatia",
    "323": "Cuba",
    ...
    "501": "French Southern Territories (Adelie Land)",
    "621": "Djibouti",
    "626": "Gabon",
    "213": "Georgia",
    "629": "Gambia",
    "443": "Palestinian Territory ( Occupied)",
    "211": "Germany",
    "218": "Germany",
    "627": "Ghana",
    "236": "Gibraltar",
    ...
    "438": "Jordan",
    "634": "Kenya",
    "445": "Korea (Democratic People's Republic of), ",
    ...
    "675": "Uganda",
    "272": "Ukraine",
    "274": "Macedonia ( the former Yugoslav Republic of)",
    "622": "Egypt",
    "232": "United Kingdom",
    "233": "United Kingdom",
    "234": "United Kingdom",
...
# Bits 37 to 39
userprottype = {
    "011": "Serial User Protocol (see bits 40 to 42)",
...
# Bits 40 to 42
serialusertype = {
    "000": "ELT with Serial Identification",
...
pselect = {
    ...
    "2": {
        ...
        "ELT": [
            ("ELT - Aircraft marking - tail", "2-011"),
...
class Hexgen:
    ...
    def beaconbaudot(self):
        ...
        if is_number(beaconno_input):
            sno = self.getbaudot(
                ...
                1,
                1,
                "Beacon number must be in range 0-9",
                "id_beaconnoerror",
            )
        else:
            self.seterror(
                "Beacon number must be numeric (range 0-9)", "id_beaconnoerror"
            )
            sno = "001101"
        return sno
    ...
    def getbaudot(self, strval, min, max, errormsg, flderror, short=False):
        ...
        for letter in strval:
            try:
                key = next(
                    key for key, value in baudot.items() if value == letter.upper()
                )
                if short:
                    key = key[1:]
                bin = bin + key
...
class Mmsi_location_protocol(Hexgen):
    ...
    def getresult(self):
        ...
        sno = self.getserial(
            ...
            0,
            15,
            "Beacon number must be numeric (range 0-15)",
            4,
            "id_beaconnoerror",
        )
        ...
        if not is_number(radio_or_mmsi_input) or len(radio_or_mmsi_input) > 6:
            ...
            self.seterror(
                "MMSI must be numeric and max 6 digits", "id_radio_or_mmsierror"
            )
        else:
            # must be a location protocol and use decimal conversion
            mmsi = dec2bin(int(radio_or_mmsi_input), 20)
            self.results["binary"] = (
                self.mid + "+" + dec2bin(int(radio_or_mmsi_input), 20)
            )
            self.sethexcode(
                "0",
                self.mid,
                self.protocol.split("-")[2],
                mmsi,
                sno,
                "0111111111",
                "01111111111",
...
class Radio_callsign(Hexgen):
    ...
    def getresult(self):
        ...
        if len(radio_input) > 7:
            self.seterror(
                "Radio call sign must not exceed 7 characters", "id_radioerror"
            )
        elif len(radio_input) > 4 and not is_number(radio_input[4:]):
            self.seterror(
                "Radio Callsign last digits need to be numeric", "id_radioerror"
            )
        else:
            bin1 = self.getbaudot(
                radio_input[:4],
                1,
                4,
                "Radio Callsign error (at least 1 digit)",
                "id_radioerror",
            )
...
class Aircraftmarking(Hexgen):
    ...
    def getresult(self):
        ...
        sno = self.getserial(
            ...
            0,
            3,
            "Beacon number must be numeric (range 0-3)",
            2,
            "id_beaconnoerror",
        )
        acrft = (7 - len(aircraftmarking_input)) * "100100" + self.getbaudot(
            aircraftmarking_input,
            1,
            7,
            "First generation aircraft marking maximum 7 characters",
            "id_aircraftmarkingerror",
        )
    ...
    def getresult(self):
        ...
        if is_number(radio_or_mmsi_input):
            if len(radio_or_mmsi_input) > 6:
                self.seterror(
                    "First generation maritime protocol maximum 6 characters",
                    "id_radio_or_mmsierror",
                )
        radio = (6 - len(radio_or_mmsi_input)) * "100100" + self.getbaudot(
            radio_or_mmsi_input,
            1,
            6,
            "MMSI or radio marking maximum 6 characters",
            "id_radio_or_mmsierror",
        )
        self.sethexcode(
            "1", self.mid, "010", radio, self.beaconbaudot(), "00", self.auxdeviceinput
        )
        return self.results
...
class Secondgen(Hexgen):
    def __init__(self, formfields, protocol):
        ...
        self.sn = self.getserial(
            ...
            0,
            16383,
            "Serial number range (0 - 16,383)",
            14,
            "id_serialnumbererror",
        )
        self.ta = self.getserial(
            tano,
            0,
            65535,
            "Type approval number range (0 - 65,535)",
            16,
            "id_tanoerror",
        )
    ...
    def getresult(self):
        ...
        if len(mmsi_input) == 0:
            ...
            mmsi = self.getserial(
                ...
                0,
                999999,
                "Serial number 6 digit maximum range (0 - 999999)",
                20,
                "id_mmsierror",
            )
        ...
        if len(epirbais_input) == 0:
            ...
            epirbais = self.getserial(
                epirbais_input,
                0,
                9999,
                "EPIRB AIS range error (0 - 9999)",
                14,
                "id_epirbaiserror",
            )
    ...
    def getresult(self):
        ...
        radio = (
            self.getbaudot(
                ...
                1,
                7,
                "Radio callsign maximum 7 characters",
                "id_radioerror",
            )
            + (7 - len(radio_input)) * "100100"
            + "00"
        )
        self.sethexcode(
            "1", self.mid, "101", self.ta, self.sn, self.testprotocol, self.ptype, radio
        )
        return self.results
...
class Aircraftmarking_secgen(Secondgen):
    ...
    def getresult(self):
        aircraftmarking_input = str(self.formfields.get("aircraftmarking_input"))
        aircraftmarking = (
            (7 - len(aircraftmarking_input)) * "100100"
            + self.getbaudot(
                aircraftmarking_input,
                1,
                7,
                "Aircraft marking maximum 7 characters",
                "id_aircraftmarkingerror",
            )
            + "00"
        )
...
class Air24bit_secgen(Secondgen):
    ...
    def getresult(self):
        ...
        sn = self.getserial(
            elt24bitaddress_serial,
            0,
            16777215,
            "Serial number range (0 - 16,777,215)",
            24,
            "id_elt24biterror",
        )
        self.sethexcode(
...
class Systemtest_secgen(Secondgen):
    ...
    def getresult(self):
        ...
        testdata = self.getbinaryinput(
            systemtest, "Binary input only - 44 bit max", 44, "id_systemtesterror"
        )
        self.sethexcode(
            "1",
            self.mid,
...
class Aircraftoperator_secgen(Secondgen):
    ...
    def getresult(self):
        ...
        acftop = self.getbaudot(
            aircraftoperator_input,
            3,
            3,
            "Aircraft operator must be 3 digits",
            "id_aircraftoperatorerror",
        )
        ...
        sn = self.getserial(
            ...
            0,
            4095,
            "Serial number range (0 - 4,095)",
            12,
            "id_aircraftserialerror",
        )
        self.sethexcode(
            "1",
            self.mid,
...
class Aircraftoperator(Hexgen):
    ...
    def getresult(self):
        ...
        ta = self.getserial(
            ...
            0,
            1023,
            "Type approval number range (0 - 1,023)",
            10,
            "id_tanoerror",
        )
        self.sethexcode(
...
class Aircraftoperator_location(Hexgen):
    ...
    def getresult(self):
        ...
        acftop = self.getbaudot(
            ...
            3,
            3,
            "Aircraft operator must be 3 alphabetic digits",
            "id_aircraftoperatorerror",
            True,
        )
        serialnumber_input = str(self.formfields.get("serialnumber_input"))
...
class Serial(Hexgen):
    ...
    def getresult(self):
        ...
        sn = self.getserial(
            ...
            0,
            1048575,
            "Serial number range (0 - 1,048,575)",
            20,
            "id_serialnumbererror",
        )
        ta = self.getserial(
...
class Air24bit_location(Hexgen):
    ...
    def getresult(self):
        ...
        self.sethexcode(
            "0", self.mid, self.protocol.split("-")[2], sn, "0111111111", "01111111111"
        )
        return self.results
...
class Serial24(Hexgen):
    ...
    def getresult(self):
        elt24bitaddress_serialuser = str(
            self.formfields.get("elt24bitaddress_serialuser")
        )
        beaconno_input = str(self.formfields.get("beaconno_input"))
        sn = self.getserial(
            elt24bitaddress_serialuser,
            0,
            16777215,
            "Serial number range (0 - 16,777,215)",
            24,
        ...
        eltno = self.getserial(
            beaconno_input,
            0,
            63,
            "Beacon identification number range must be 0 - 63",
            6,
            "id_beaconnoerror",
        )
        if self.tano == "0":
...
class National_location(Hexgen):
    ...
    def getresult(self):
        ...
        nationallocation = self.getserial(
            nationallocation_input,
            0,
            262143,
            "Maximum value (262,143) exceeded",
            18,
            "id_nationallocationerror",
        )
        self.sethexcode(
            "0",
            self.mid,
...
posneg = {"0": "-1", "1": "1"}
dataflag110 = {
    "0": "Result of bits 113 to 132 defined nationally",
...
homer = {
    "0": "121.5 MHz Homing device not present",
    "1": "121.5 MHz Homing device present",
}
...
beacon_rls = {
    "00": "No automatic RLM Type-1 received - No manual RLM Type 2 received",
...
vessel_id = {
    "000": "No aircraft or maritime identity (may be for national use; default bits 94-137 all 0)",
    "001": "Maritime MMSI",
    "010": "Radio call sign",
