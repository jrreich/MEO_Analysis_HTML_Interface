...
import re
import my_packages.beacon as bcn
...
def time_to_datetime(timein):
    ...
    if isinstance(timein, str):
        ...
        try:
            ...
            pass
        try:
            return datetime.strptime(timein, GMT_format)
        except:
            TypeError("timein string does not match available formats")
...
def filter_range(df, field, criteria_low, criteria_high):
    return df[
        ((df[field] >= float(criteria_low)) & (df[field] <= float(criteria_high)))
    ]
...
def singleburst_loc(df, lat_GT, lon_GT, MEOLUT):
    ...
    df2["Error_GT"] = df2[["latitude", "longitude", "Lat_GT", "Lon_GT"]].apply(
        haversine, axis=1
    )
    df2["Error_Enc"] = df2[["latitude", "longitude", "Enc_Lat", "Enc_Lon"]].apply(
        haversine, axis=1
    )
    ...
    dfSB = dfSB[
        (dfSB.timestart_diff > pd.Timedelta(seconds=5)) | (dfSB.timestart_diff.isnull())
    ]  ###PROBLEM
    return dfSB
...
def multiburst_loc(df, lat_GT, lon_GT, MEOLUT, window_span):
    ...
    df3 = df3.sort_values(
        "timelast"
    )  # Rethink how you do windows 12/9 -- I think it was solved previously 1/8/17
    ...
    df3["last_in_window"] = (df3.timestart_diff > pd.Timedelta(minutes=9)) & (
        df3.timestart_diff2 > pd.Timedelta(minutes=3)
    )
    df3 = df3[
        (
            (df3.timestart_diff > pd.Timedelta(minutes=9))
            & (df3.timestart_diff2 > pd.Timedelta(minutes=3))
        )
        | df3.timestart_diff.isnull()
    ]
    return df3
...
def write_headers(filetype, csvoutfile, approot, J1_header):
    with open(os.path.join(approot, csvoutfile), "w", newline="") as csvfile:
        ...
        if filetype == "packet" or filetype == "TOA_FOA_DATA":
            wr.writerow(J1_header)
        elif filetype == "status":
            wr.writerow(Statusfileheader)
    return None
        ...
        for event, element in etree.iterparse(filename, tag="TOA_FOA_DATA"):
            if country == "Canada":
                ...
                for child in element:
                    if child.tag == "MF22" and child.text == beaconId:
...
def write_bursts_csv(
    packets, csvoutfile, filetypesearch, filename, approot, bcnId=False
):
    burst = list()
    componentlist = list()
    with open(os.path.join(approot, csvoutfile), "a", newline="") as csvfile:
        csvoutwriter = csv.writer(csvfile, delimiter=",", quoting=csv.QUOTE_MINIMAL)
        if filetypesearch == "packet" or filetypesearch == "TOA_FOA_DATA":
...
def find_file_inzip(zipfilename, searchstring):
    with zipfile.ZipFile(zipfilename, "r") as myzip:
        ...
        print(
            "returning "
            + str(len(matches))
            + " files in zip with search string "
            + searchstring
        )
        return matches
...
def find_packets(
    ...
    **kwargs
    ...
    sql_query = (
        "SELECT UplinkTOADate, UplinkFOA from MeolutPackets "
        "WHERE "
        "BcnId15 like ? "
        "AND "
        "MeolutId like ? "
...
def plot_packets(
    packets, MEOLUT, ant_i, start_time, end_time, packets_found, percent_packets
):
    global ax1
    frequencylist = list()
    timelist = list()
    ...
    plt.title(
        "{} - antenna {} -- {} packets = {:.1f}%".format(
            MEOLUT, ant_i, packets_found, percent_packets
        )
    )
...
def xlx_analysis(
    ...
    **kwargs
    ...
    if "Long" in kwargs:
        Long_GT = kwargs["Long"]
        MEO_dist = haversine(
            [Lat_GT, Long_GT, MEOLUTLoc[MEOLUT][0], MEOLUTLoc[MEOLUT][1]]
        )
    else:
        Long_GT = None
        MEO_dist = "NA"
    df = pd.read_excel(
        InputMEO_excelfile, index_col="TimeSolutionAdded"
    )  # , parse_dates = True) #, parse_cols =  data_cols) #parse_dates = True,
    df["TimeFirst"] = pd.to_datetime(
        df["TimeFirst"], errors="ignore"
    )  ### FIX THIS WHEN TimeFirst is already a datetime.time 12/9
    df["TimeLast"] = pd.to_datetime(df["TimeLast"], errors="ignore")
    df["TimeSolutionGenerated"] = pd.to_datetime(
        df["TimeSolutionGenerated"], errors="ignore"
    )
    ...
    window_time = (
        20  # minutes - no longer used since now just look for change in TimeFirst
    )
    ...
    outfilelist = list()
    outfiletag = "{}_{}_{:%Y-%m-%d-%H%M}_{:%Y-%m-%d-%H%M}".format(
        MEOLUT, BeaconID, TimeStart, TimeEnd
    )
    ...
    with open(OUTfile, "w", newline="") as csvfile:
        ...
        csvoutwriter.writerow(
            ["Distance From MEOLUT", MEO_dist]
        )  #'{:.2f}'.format(MEO_dist)])
        csvoutwriter.writerow([])
        csvoutwriter.writerow(
            ["Time First Burst", "{:%Y-%m-%d %H:%M:%S}".format(timefirst)]
        )
        csvoutwriter.writerow(
            ["Time Last Burst", "{:%Y-%m-%d %H:%M:%S}".format(timelast)]
        )
        ...
        csvoutwriter.writerow(
            ["% SBL <10km (vs Enc)", "{:.2%}".format(prob_SBL_10_enc)]
        )
        csvoutwriter.writerow([])
        csvoutwriter.writerow(["NumSBL <20km (vs Enc)", SBL_20_enc])
        csvoutwriter.writerow(
            ["% SBL <20km (vs Enc)", "{:.2%}".format(prob_SBL_20_enc)]
        )
        csvoutwriter.writerow([])
        ...
        csvoutwriter.writerow(
            ["% MBL <10km (vs Enc)", "{:.2%}".format(prob_MBL_10_enc)]
        )
        csvoutwriter.writerow([])
        csvoutwriter.writerow(["NumMBL <20km (vs Enc)", MBL_20_enc])
        csvoutwriter.writerow(
            ["% MBL <20km (vs Enc)", "{:.2%}".format(prob_MBL_20_enc)]
        )
                ...
                for row in filereader:
                    if row[17] != "":
                        pntEnc = folEnc.newpoint(
                            ...
                            description="Encoded Location - Beacon = "
                            + row[2]
                            + "\n\nTimeSolutionAdded = "
                            + row[0]
                            + "\nLat,Long = ("
        ...
        with open(MBLfile, "r") as csvfile:
            ...
            for row in filereader:
                pntMBL = folMBL.newpoint(
                    coords=[(float(row[8]), float(row[7]), float(row[9]))],
                    description="Multi Burst Location - Beacon = "
                    + row[2]
                    + "\n\nTimeSolutionAdded = "
                    + row[0]
                    + "\nTimeFirst = "
    ...
    if "LEOgen" in result:
        ...
        if df.empty:
            ...
            with open(LEOoutfile, "r") as csvfile:
                ...
                for row in filereader:
                    pnt_LEO = fol_LEO.newpoint(
                        coords=[(float(row[22]), float(row[21]))],
...
def parse_results(result, MEOList):
    ...
    for MEO in MEOList:
        MEO_dist.append(
            haversine([Lat_GT, Long_GT, MEOLUTLoc[MEO][0], MEOLUTLoc[MEO][1]])
        )
...
def MSSQL_analysis(
    ...
    **kwargs
    ...
    OUTPUTFOLDER, approot, servername, databasename = (
        config_dict["OUTPUTFOLDER"],
        config_dict["approot"],
        config_dict["servername"],
        config_dict["oppsdatabase"],
    )
    if "sql_login" in kwargs:
        conn = odbc.connect(
            r"Driver={FreeTDS};Server="
    ...
    if MEOLUTList == None:
        MEOLUTList = MEOList
    BeaconQuery, BeaconID, Lat_GT, Long_GT, Location, MEO_dist = parse_results(
        result, MEOLUTList
    )
    sql_query = (
        "SELECT * FROM "
    ...
    df = pd.read_sql_query(
        sql_query, conn, index_col="timesolutionadded", params=params
    )
    df_data = []
    ...
    if "KMLgen" in result:
        ...
        if Lat_GT != 0:
            ...
            polycircle1 = polycircles.Polycircle(
                latitude=Lat_GT,
                longitude=Long_GT,
                radius=firstrad,
                number_of_vertices=36,
            )
            pol1 = kml.newpolygon(name="5km", outerboundaryis=polycircle1.to_kml())
            pol1.style.polystyle.color = simplekml.Color.changealphaint(
                100, simplekml.Color.green
            )
            # Second region

            polycircle2outer = polycircles.Polycircle(
                latitude=Lat_GT,
                longitude=Long_GT,
                radius=secondrad,
                number_of_vertices=36,
            ...
            pol2.style.polystyle.color = simplekml.Color.changealphaint(
                100, simplekml.Color.yellow
            )
            pntEnc = folEnc.newpoint(
                name="Ground Truth",
        ...
        if "LEOGen" in result:
            ...
            sql_query = (
                "SELECT b.AddTime, a.* from Lut406Solution a, inputmessage b "
                    ...
                    for row in filereader:
                        if row[18]:
                            pnt_RCC = fol_RCC.newpoint(
                                ...
                                "\nLat,Long = {0:.2f}".format(float(row[18]))
                                + ",{0:.2f}".format(float(row[19]))
                                + "\nAddTime = "
                                + row[1]
                                + "\nAlertSiteSolId = "
                                + row[2]
...
def MSSQL_burst(arg_dict, config_dict, **kwargs):
    servername, databasename, OUTPUTFOLDER, approot = (
        config_dict["servername"],
        config_dict["oppsdatabase"],
        config_dict["OUTPUTFOLDER"],
        config_dict["approot"],
    ...
    for MEOLUT in arg_dict["MEOLUTList"]:
        for ant_i in ant_list:
            if arg_dict.get("UseBeaconID") == "UseRefBeacon":
                ...
                if arg_dict.get("refbeacon") == "FL-HI-Ref":
                    ...
                    beaconIDstring = ReferenceBeacons[arg_dict.get("refbeacon")][
                        "beaconId"
                    ]
            else:
                beaconIDstring = arg_dict.get("beaconID", "%")
            outpackets = find_packets(
                servername,
                databasename,
                beaconIDstring,
                arg_dict["StartTime"],
        ...
        filename = os.path.normpath(
            os.path.join(OUTPUTFOLDER, str(MEOLUT) + "_" + filetimetag + "_output.png")
        )
...
def MEOLUT_alarms(
    StartTime, EndTime, servername="localhost", databasename="mccoperational", **kwargs
):
    if "sql_login" in kwargs:
        conn = odbc.connect(
            r"Driver={FreeTDS};Server="
    ...
    sql_query = "SELECT * from MeolutAlarms " "WHERE " "AddTime BETWEEN ? AND ? "
    ...
    for alarm in closedalarms:
        alarms = {
            ...
            "closedate": dfClosed[
                (dfClosed.alarmid == alarm) & (dfClosed.alarmtimeon.isnull())
            ]
            .iloc[0]
            .alarmtimeoff,
        }
...
def MEOLUT_status(
    ...
    query_params = [StartTime, EndTime]
    sql_query = "SELECT * from MeolutStatus " "WHERE " "AddTime BETWEEN ? AND ? "
    df = pd.read_sql_query(sql_query, conn, index_col="msgtime", params=query_params)
    df.sort_index().ix[StartTime:EndTime]
    if df.empty:
        HI = {
...
def MEOLUT_percent(
    StartTime, EndTime, servername="localhost", databasename="MccTestLGM", **kwargs
):
    if "sql_login" in kwargs:
        conn = odbc.connect(
            r"Driver={FreeTDS};Server="
    ...
    def get_packets(BeaconID, MEOLUT, Antenna, StartTime, EndTime):
        ...
        sql_query = (
            "SELECT COUNT(*) from MeolutPackets "
            "WHERE "
            "BcnId15 like ? "
            "AND "
            "MeolutId like ? "
...
def Open_Sites(servername="localhost", databasename="MccTestLGM", **kwargs):
    ...
    params = ["N"]
    sql_query = (
        "SELECT alertsitenum, bcnid15, midname, "
...
def alertsitesol_query(
    ...
    **kwargs
    ...
    params = [sitenum]
    sql_query = (
        "select alertsitesolid, inputdatatype, bcnid15, framesync, "
                ...
                if (row[16] != "NULL") or row[18]:
                    if row[16] != "NULL":
                        pnt_RCC = fol_RCC.newpoint(
                            ...
                            "\nLat,Long = {0:.4f}".format(float(row[16]))
                            + ",{0:.4f}".format(float(row[17]))
                            + "\nAddTime = "
                            + str(row[8])
                            + "\nAlertSiteSolId = "
                            + str(int(row[0]))
                        ...
                        pnt_RCC = fol_RCC.newpoint(
                            ...
                            "\nLat,Long = {0:.4f}".format(float(row[18]))
                            + ",{0:.4f}".format(float(row[19]))
                            + "\nAddTime = "
                            + str(row[8])
                            + "\nAlertSiteSolId = "
                            + str(int(row[0]))
    ...
    **kwargs
    ...
    if "makeKML" in kwargs:
        if kwargs.get("makeKML"):
            ...
            fol_RCC = kml.newfolder(name="RCC Output Locations")
            for row in outsols:
                if ((row[17] is not None) and (row[17] != "NULL")) or (
                    (row[19] is not None) and (row[19] != "NULL")
                ):
                    if (row[17] is not None) and (row[17] != "NULL"):
                        pnt_RCC = fol_RCC.newpoint(
                            coords=[(float(row[18]), float(row[17]))],
                            description="RCC Solution \nBeacon = " + str(row[26]) + "\n"
                            "\nLat,Long = {0:.4f}".format(float(row[17]))
                            + ",{0:.4f}".format(float(row[18]))
                            + "\nAddTime = "
                            + str(row[0])
                            + "\nAlertSiteSolId = "
                            + str(int(row[1]))
                    ...
                    if (row[19] is not None) and (row[19] != "NULL"):
                        pnt_RCC = fol_RCC.newpoint(
                            coords=[(float(row[20]), float(row[19]))],
                            description="RCC Solution \nBeacon = " + str(row[26]) + "\n"
                            "\nLat,Long = {0:.4f}".format(float(row[19]))
                            + ",{0:.4f}".format(float(row[20]))
                            + "\nAddTime = "
                            + str(row[0])
                            + "\nAlertSiteSolId = "
                            + str(int(row[1]))
...
def both_kml(
    ...
    **kwargs
    ...
    for row in insols:
        if ((row[16] is not None) and (row[16] != "NULL")) or (
            (row[18] is not None) and (row[18] != "NULL")
        ):
            if (row[16] is not None) and (row[16] != "NULL"):
                pnt_MEO = fol_MEO.newpoint(
                    coords=[(float(row[17]), float(row[16]))],
                    description="MEO Solution \nBeacon = " + str(row[2]) + "\n"
                    "\nLat,Long = {0:.4f}".format(float(row[16]))
                    + ",{0:.4f}".format(float(row[17]))
...
def MSSQL_beacon_analysis(
    ...
    **kwargs
    ...
    outfiletag = "_{}_{}_{:%Y-%m-%d-%H%M}_{:%Y-%m-%d-%H%M}".format(
        BeaconID, MEOList_str, TimeStart, TimeEnd
    )
    ...
    def hist_cum_plot(
        df, list1, maxrange, title, textstr, legendhandles, textstr2, **args
    ):
        ...
        if list1:
            for i, df1 in enumerate(list1):
                ...
                count, division = np.histogram(
                    df2, bins=bin_range, range=(bin_range.min(), bin_range.max())
                )
                cumulative = np.cumsum(count).astype(np.float64)
                x = np.linspace(0, bin_range[-1], cumulative.size)
                cumulative /= cumulative.max()
    ...
    )  # , index_col = 'timesolutionadded', params=params)
    df_data = []
    for MEO in MEOLUTList:
        df_data.append(filter1(df, "sourceid", MEO))
    dfMEO = pd.concat(df_data)
    ...
    if result.get("filter1check", False):
        if result.get("filter1sel") == "filter1ran":
            df_filter = filter_range(
                df_filter,
                fieldname[result.get("filter1")],
                result.get("filter1rangelow", 0),
                result.get("filter1rangehigh", 100),
            )
            filterlist.append(
                fieldname[result.get("filter1")]
                + " between "
            ...
            df_filter = filter1(
                df_filter,
                fieldname[result.get("filter1")],
                result.get("filter1value", 0),
            )
            filterlist.append(
                fieldname[result.get("filter1")] + " = " + result.get("filter1value", 0)
            )
    if result.get("filter2check", False):
        if result.get("filter2sel") == "filter2ran":
            df_filter = filter_range(
                df_filter,
                fieldname[result.get("filter2")],
                result.get("filter2rangelow", 0),
                result.get("filter2rangehigh", 100),
            )
            filterlist.append(
                fieldname[result.get("filter2")]
                + " between "
            ...
            df_filter = filter1(
                df_filter,
                fieldname[result.get("filter2")],
                result.get("filter2value", 0),
            )
            filterlist.append(
                fieldname[result.get("filter2")] + " = " + result.get("filter2value", 0)
            )
    if result.get("filter3check", False):
        if result.get("filter3sel") == "filter3ran":
            df_filter = filter_range(
                df_filter,
                fieldname[result.get("filter3")],
                result.get("filter3rangelow", 0),
                result.get("filter3rangehigh", 100),
            )
            filterlist.append(
                fieldname[result.get("filter3")]
                + " between "
            ...
            df_filter = filter1(
                df_filter,
                fieldname[result.get("filter3")],
                result.get("filter3value", 0),
            )
            filterlist.append(
                fieldname[result.get("filter3")] + " = " + result.get("filter3value", 0)
            )
    if df_filter.empty:
        print("no data ")
    ...
    if (Lat_GT is not None) and (Long_GT is not None):
        ...
        df_filter["Error_GT"] = df_filter[
            ["latitude", "longitude", "Lat_GT", "Lon_GT"]
        ...
        df_gt = df_gt.iloc[::10]
        df_gt = df_gt.loc[
            df_gt.index > TimeStart
        ]  # sol_starttime - timedelta(minutes=10)]
        ...
        def get_lat_lon_time(df, dateandtime):
            if dateandtime not in df.index:
                ...
                df = df.append(newrow)
            LON = (
                pd.Series(index=df.index, data=df.lon.values)
                .sort_index()
                .interpolate(method="time")[dateandtime]
            )
            LAT = (
                pd.Series(index=df.index, data=df.lat.values)
                .sort_index()
                .interpolate(method="time")[dateandtime]
            )
        ...
        df_filter["Error_GT"] = df_filter[
            ["Lat_GT", "Lon_GT", "latitude", "longitude"]
        ].apply(haversine, axis=1)
        if "Enc_Lat" in df_filter.columns:
            df_filter["Error_Enc_Loc_GT"] = df_filter[
                ["Lat_GT", "Lon_GT", "Enc_Lat", "Enc_Lon"]
            ].apply(haversine, axis=1)
    ...
    df_filter["Error_Enc"] = df_filter[
        ["latitude", "longitude", "Enc_Lat", "Enc_Lon"]
    ].apply(haversine, axis=1)
    ...
    if result.get("SolutionsOut", False) or result.get("KMLgen", False):
        Solutionfilename = os.path.normpath(
            os.path.join(OUTPUTFOLDER, filetimetag + "_Solutions.csv")
        )
    ...
    if result.get("histcum", False):
        textstr1 = (
            "Total Stats \n\nNum Locations = %.d\nmean = %.1f km\nmedian = %.1f km\n95%% = %.1f km\n\n%% within 5 km = %.1f%%\n%% within 10 km = %.1f%%"
            % (statsout[0], mean, median, q95, 100 * win_5km, 100 * win_10km)
        ...
        if result.get("beaconLat") and result.get("beaconLon"):
            titleheader.append(
                "(" + result.get("beaconLat") + ", " + result.get("beaconLon") + ")"
            )
        if result.get("UseBeaconID") == "UseRefBeacon":
            titleheader.append(
                "Ref Beacon: " + result.get("refbeacon") + " - " + BeaconID + "\n"
            )
            titleheader.append(
                "Location: ("
                + str(ReferenceBeacons[result.get("refbeacon")]["beaconLat"])
                + ", "
                + str(ReferenceBeacons[result.get("refbeacon")]["beaconLat"])
                + ")"
            )
            ...
            if varyfield == "dop":
                ...
                for i in doprange:
                    if i == doprange[-1]:
                        ...
                        dflist.append(
                            filter_range(df_filter, varyfield, i, i + doprangediff)
                        )
                        legendhandles.append(
                            varyfield + ": " + str(i) + " - " + str(i + doprangediff)
                        )
            if varyfield == "expectedhorzerror":
                ...
                for i in eherange:
                    if i == eherange[-1]:
                        ...
                        dflist.append(
                            filter_range(df_filter, varyfield, i, i + eherangediff)
                        )
                        legendhandles.append(
                            varyfield + ": " + str(i) + " - " + str(i + eherangediff)
                        )
            if varyfield == "numbursts":
                numburstrangediff = 3
                numburstrange = [1, 2]
                numburstrange.extend(
                    list(range(3, 30, numburstrangediff))
                )  # could be variable
                for i in numburstrange:
                    if i == 1 or i == 2:
                        ...
                        dflist.append(
                            filter_range(df_filter, varyfield, i, i + numburstrangediff)
                        )
                        legendhandles.append(
                            varyfield
                            + ": "
                            + str(i)
                            + " - "
                            + str(i + numburstrangediff)
                        )
            if varyfield == "numpackets":
                numpacketsrangediff = 10
                numpacketsrange = list(
                    range(0, 100, numpacketsrangediff)
                )  # could be variable
                for i in numpacketsrange:
                    if i == numpacketsrange[-1]:
                        ...
                        dflist.append(
                            filter_range(
                                df_filter, varyfield, i, i + numpacketsrangediff
                            )
                        )
                        legendhandles.append(
                            varyfield
            ...
            if varyfield == "averagecn0":
                ...
                for i in cn0range:
                    if i == cn0range[-1]:
                        ...
                        dflist.append(
                            filter_range(df_filter, varyfield, i, i + cn0rangediff)
                        )
                        legendhandles.append(
                            varyfield + ": " + str(i) + " - " + str(i + cn0rangediff)
                        )
        ...
        plt1 = hist_cum_plot(
            df_filter, dflist, 50, title, textstr1, legendhandles, filtertext
        )

        filename = os.path.normpath(
            os.path.join(
                OUTPUTFOLDER, "histogram" + outfiletag + "_" + filetimetag + ".png"
            )
        )
        ...
        plt.cla()
        plt.close("all")
    statsfile = os.path.normpath(
        os.path.join(OUTPUTFOLDER, "out_stats_sum" + filetimetag + "_out_stats_sum.csv")
    )
    rw = []
    rw.extend(statsout)
        ...
        with open(os.path.join(approot, Solutionfilename), "r") as csvfile:
            ...
            for row in filereader:
                ...
                if result.get("ErrorLines") and (GT_error != "NA"):
                    ...
                    GT_errorline.coords = [
                        (float(row[8]), float(row[7]), float(row[9])),
                        (float(row[20]), float(row[19]), 0),
                    ]
                    GT_errorline.timespan.begin = (
                        str(row[6])[:10] + "T" + str(row[6])[11:19]
                    )
...
def MeoDataCollection(
    result, MEOLUTList, StartTime, EndTime, config_dict, zip_file=False, **kwargs
):
    servername, databasename, OUTPUTFOLDER, approot = (
        config_dict["servername"],
        config_dict["oppsdatabase"],
    ...
    if result.get("J1data", False):
        ...
        out = c.fetchall()
        if out:
            J1_file = os.path.join(
                OUTPUTFOLDER, "J1_" + meoListString + outfiletag + ".csv"
            )
        ...
        if zip_file:
            J1_file_zip = os.path.join(
                OUTPUTFOLDER, "J1_from_zip_" + meoListString + outfiletag + ".csv"
            )
    ...
    if result.get("J2data", False):
        ...
        out = c.fetchall()
        if out:
            J2_file = os.path.join(
                OUTPUTFOLDER, "J2_" + meoListString + outfiletag + ".csv"
            )
    ...
    if result.get("J3data", False):
        ...
        out = c.fetchall()
        if out:
            J3_file = os.path.join(
                OUTPUTFOLDER,
                "J3_"
                + meoListString
                + "_{:%Y-%m-%d-%H%M}_{:%Y-%m-%d-%H%M}".format(StartTime, EndTime)
                + ".csv",
            )
...
def api_meo_accuracy_all(config_dict, arg_dict, output_format="data_list"):
    ...
    sql_query = """\
        ...
        """
    params = [
        datetime.strftime(arg_dict.get("StartTime"), datetime_format),
        datetime.strftime(arg_dict.get("EndTime"), datetime_format),
    ]
    ...
    if out:
        ...
        out_file_string = (
            meoListString
            + "_{:%Y-%m-%d-%H%M}_{:%Y-%m-%d-%H%M}.csv".format(
                arg_dict.get("StartTime"), arg_dict.get("EndTime")
            )
        )
        output = sql_outputer(
            out,
...
def api_meo_ref_beacon_locations(arg_dict, config_dict, output_format="json"):
    ...
    for j in cur:
        out.append(j)
    if out:
        out_file_string = "{:%Y-%m-%d-%H%M}_{:%Y-%m-%d-%H%M}.csv".format(
            arg_dict.get("StartTime"), arg_dict.get("EndTime")
        )
        output = sql_outputer(
            out,
...
def api_meo_schedule(MEOLUTList, StartTime, EndTime, output_format, config_dict):
    ...
    if out:
        ...
        out_file_string = (
            "J3_"
            + meoListString
            + "_{:%Y-%m-%d-%H%M}_{:%Y-%m-%d-%H%M}.csv".format(StartTime, EndTime)
        )
        return sql_outputer(
            out,
            output_format,
...
def sql_outputer(outdata, output_format, c, one=False, **kwargs):
    columns = [column[0] for column in c.description]
    if output_format == "csv":
        csv_out_file = os.path.join(
            kwargs.get("OUTPUTFOLDER") + kwargs.get("out_file_string")
        )
        with open(
            os.path.join(kwargs.get("approot"), csv_out_file), "w", newline=""
        ) as csvfile:
            csvoutwriter = csv.writer(csvfile, delimiter=",", quoting=csv.QUOTE_MINIMAL)
            csvoutwriter.writerow(columns)
            for row in outdata:
    ...
    if output_format == "json":
        r = [
            dict((c.description[i][0], value) for i, value in enumerate(row))
            for row in outdata
        ]
        return (r[0] if r else None) if one else r
    else:
...
def api_meo_ref_beacon_accuracy(
    MEOLUT, StartTime, EndTime, arg_dict, config_dict, **kwargs
):
    ...
    output = {
        ...
        "beaconLat": beacon["beaconLat"],
        "beaconLon": beacon["beaconLon"],
    }
    if not out:
        output.update(
            {
        ...
        output.update(
            {
                "p75": np.percentile(out_distlist, 75),
        ...
        for i, dist in enumerate(distlist):
            ...
            output.update(
                {
                    keylist[i]: (100.0 * len([x for x in out_distlist if x < dist]))
                    / num_of_locations,
                    numlist[i]: len([x for x in out_distlist if x < dist]),
                }
            )
        output.update(
            {"percent_locations": 100.0 * len(out_distlist) / expected_locations}
        )
        return output
...
def api_meo_location_accuracy(MEOLUT, StartTime, EndTime, config_dict, **kwargs):
    ...
    output[MEOLUT]["percent_locations"] = (
        100.0 * output[MEOLUT]["num_of_locations"] / (float(timedelta_secs / 50))
    )
    sql_query = """\
        SELECT DISTANCE
        FROM [MccMeoLutMonitor].[dbo].[InputMeoSolution] 
    ...
    for dist in distList:
        ...
        output[MEOLUT]["number_less_than_" + str(dist) + "km"] = sum(
            i[0] < dist for i in out
        )
        output[MEOLUT]["percent_less_than_" + str(dist) + "km"] = (
            100.0 * sum(i[0] < dist for i in out)
        ) / float(output[MEOLUT]["num_of_locations"])
    return output
...
def real_time_packet_stats(MEOLUT, Time, config_dict):
    ...
    columns = [column[0] for column in c.description]
    r = [
        dict((c.description[i][0], str(value)) for i, value in enumerate(row))
        for row in out
    ]
    return r[0] if r else None


def api_meo_packet_throughput(
    MEOLUT, StartTime, EndTime, config_dict, rep_rate, minutes, **kwargs
):
    servername, databasename = config_dict["servername"], config_dict["oppsdatabase"]
    if "sql_login" in kwargs:
        conn = odbc.connect(
    ...
    for j in out:
        ...
        if rep_rate:
            output["antenna"][j[0]]["percent"] = (
                100.0 * float(j[1]) / float(timedelta_secs / float(rep_rate))
            )
...
def api_site_sum_query(arg_dict, config_dict, **kwargs):
    ...
    if arg_dict.get("sitenum"):
        sql_query = """ select top 1 {} FROM 
            AlertSiteSum WHERE 
            AlertSiteNum = ? 
            ORDER BY TimeLast """.format(
            sql_query_field_list
        )
    ...
    if arg_dict.get("StartTime") and not NOMAS:
        if sql_query.find("WHERE") == -1:
            sql_query += " WHERE LastUpdTime > ?"
        else:
            sql_query += " AND LastUpdTime > ?"
        params.append(arg_dict["StartTime"])
    if arg_dict.get("EndTime") and not NOMAS:
        if sql_query.find("WHERE") == -1:
            sql_query += " WHERE TimeFirst < ?"
        else:
            sql_query += " AND TimeFirst < ? "
    ...
    columns = [column[0] for column in c.description]
    if out:
        out_file_string = "_SiteSum_{:%Y-%m-%d-%H%M}_{:%Y-%m-%d-%H%M}.csv".format(
            arg_dict.get("StartTime"), arg_dict.get("EndTime")
        )
        output = sql_outputer(
            out,
            arg_dict["output_format"],
            c,
            approot=approot,
            OUTPUTFOLDER=OUTPUTFOLDER,
            out_file_string=out_file_string,
...
def api_site(config_dict, sitenum=None, table=None, type="all", one=False, **kwargs):
    ...
    if "data" in kwargs:
        if not table:
            table = data.get("table", "alertsitesol")
        if not sitenum:
            sitenum = data.get("sitenum", None)
    sql_query = (
        """ SELECT * FROM [MccMeoLutMonitor].[dbo].[%s] 
        WHERE AlertSiteNum = ?
        """
        % table
    )
...
def czml_all_sites_sum_query(
    servername="localhost", databasename="mccoperational", **kwargs
):
    if "sql_login" in kwargs:
        conn = odbc.connect(
            r"Driver={FreeTDS};Server="
            ...
            idlist.append(
                {
                    ...
                    "position": {"cartographicDegrees": [float(i[7]), float(i[6]), 0]},
                    "point": {
...
def czml_meolut_ant_per(
    ...
    **kwargs
    ...
    sql_query_field_list = ", ".join(fieldlist)
    sql_query = (
        "select top 1 " + sql_query_field_list + " FROM "
...
def czml_site_meo_input(
    Sitenum, servername="localhost", databasename="mccoperational", **kwargs
):
    if "sql_login" in kwargs:
        conn = odbc.connect(
            r"Driver={FreeTDS};Server="
            ...
            idlist.append(
                {
                    ...
                    "parent": Sitenum,
                    "availability": i[5].isoformat()
                    + "Z/"
                    + LastUpdTime[0].isoformat()
                    + "Z",
                    "label": {
                        "text": str(int(i[0])),
                        "show": False,
                    ...
                    "point": {
                        ...
                        "pixelSize": 5,
                        "outlineColor": {"rgba": [233, 127, 21, 255]},
                        "outlineWidth": 2,
                    },
                    "description": "<br>".join(
                        ["Site : " + str(int(Sitenum))]
                        + [
                            str(fieldlist[j]) + " : " + str(value)
                            for j, value in enumerate(i)
                        ]
        ...
        idlist.append(
            {
                ...
                "parent": str(sitenum),
                "availability": time_to_datetime(solution["addtime"]).isoformat()
                + "Z/"
                + (time_to_datetime(solution["rcvtime"]) + delta).isoformat()
                + "Z",
                "label": {
                    "text": str(solid),
                    "show": False,
...
def czml_meo_orbit_all(starttime, endtime, config_dict, satnum=False, **kwargs):
    ...
    for sat in out:
        if sat[0] is not None:
            ...
            sql_query2 = (
                "select Epoch, SatXPos, SatYPos, SatZPos, SatXVel, SatYVel, SatZVel from OrbitVector WHERE sat = ? "
        ...
        if satdict[value]:
            if str(value)[0] == "3":
                pointprop = {
                    "color": {"rgba": [0, 0, 255, 255]},
                    "pixelSize": 5,
                    "outlineColor": {"rgba": [0, 127, 255, 200]},
                    "outlineWidth": 1,
                }
            else:
                pointprop = {
            ...
            satlist.append(
                {
                    ...
                    "availability": starttime.isoformat()
                    + "Z/"
                    + endtime.isoformat()
                    + "Z",
                    "position": {
...
def czml_meo_ant_per(data, StartTime, EndTime):
    czml_list = [
        {"id": "document", "version": "1.0", "name": "MEOLUT Antenna Percentages"}
    ]
    id_list = []
        ...
        if key == "antenna":
            for ant, stat in val.items():
                ...
                if 0.5 < stat["percent"] < 0.8:
                    rgba = [
                        (stat["percent"] - 1.7) * (-212.5),
                        (stat["percent"] - 0.5) * 850,
                        0,
                        100,
                    ]
                    rgb = (
                ...
                czml_list.append(
                    {
                        ...
                        "position": {
                            "cartographicDegrees": [
                                MEOLUT_antenna_locations[MEOLUT][ant][1],
                        ...
                        "description": "<p style='color:rgb"
                        + str(rgb)
                        + "'>"
                        + "percent = "
                        + "{:.1%}".format(stat["percent"])
                        + "</p>",
                    }
                )
    return czml_list
    ...
    **kwargs
    ...
    for sat in out:
        if sat[0] is not None:
            params = [sat[0], starttime, endtime]
            sql_query2 = (
                "select Epoch, SatXPos, SatYPos, SatZPos, SatXVel, SatYVel, SatZVel, Orbit from LutOrbitVectorOut WHERE sat = ? "
                "AND Epoch > ? "
                "AND Epoch < ? "
            )
            c2 = conn.cursor()
...
def czml_meo_sched_all(
    ...
    **kwargs
    ...
    for LUT in [3669, 3385]:
        ...
        sql_query = (
            "select LutId, AntennaId, SatId, AOSStr, LOSStr, TimeRecordAdded from PassesPlanned "
