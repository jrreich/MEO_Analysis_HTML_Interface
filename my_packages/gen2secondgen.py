...
import my_packages.gen2functions as Func
import my_packages.gen2rotating as rotating
import my_packages.definitions
...
class SecondGen(Gen2Error):
    ...
    def processHex(self, strhex):
        ...
        if len(self.bits) == 252 or len(self.bits) == 204:
                ...
                if pbit == "10":
                    self.tablebin.append(
                        [
                            ...
                            "",
                            "",
                            "This is a self-test beacon transmission or bad message",
                        ]
                    )
                else:
                    self.tablebin.append(
                        [
                            ...
                            "",
                            "",
                            "ERROR! Value is not self-test beacon transmission.",
                        ]
                    )
            ...
            if self.tac < 10000:
                warn = "<strong>{}</strong>  <br>WARNING!: SGB specifications stipulate TAC No >=10,000".format(
                    self.tac
                )
            else:
                warn = self.tac
            ...
            if Func.checkones(self.bits[141:155]) and not Func.checkones(
                ...
                self.tablebin.append(["141-154", self.bits[141:155], "Spare bits", e])
                ...
                self.errors.append(e)
                self.tablebin.append(
                    ["141-154", self.bits[141:155], "Cancellation message status:", e]
                )
            ...
            if len(self.bits) == 251:
                ...
                self.tablebin.append(["Calculated", self.calculatedBCH, "Computed", ""])
                ...
                bcherr = self.BCHerrors = Func.errors(
                    self.calculatedBCH, self.bits[203:]
                )
                if bcherr > 0:
            ...
            self.tablebin.append(
                ["1", self.bits[1], "should be 1", ["ERROR", "OK"][int(self.bits[1])]]
            )
            if self.bits[1] == "0":
                self.validhex = False
            ...
            if self.tac < 10000:
                warn = "<strong>{}</strong><br> WARNING! SGB specifications requires TAC No >=10,000".format(
                    self.tac
                )
                self.validhex = False
            else:
            ...
            self.tablebin.append(
                ["31-44", self.bits[35:45], "Serial Number", str(self.serialNum)]
            )

            self.testprotocol = Func.testProtocol(self.bits[45])
            self.tablebin.append(
                ["45", self.bits[45], "Test protocol flag:", str(self.testprotocol)]
            )
            ...
            if self.bits[61:] == "0" * 32:
                ...
                self.tablebin.append(
                    [
                        ...
                        self.bits[49:61],
                        "Partial vessel ID",
                        "WARNING!! = No Identification information or Truncated SGB 15Hex UIN (incomplete partial vessel ID. ",
                    ]
                )
    ...
    def vesselIDfill(self, deduct_offset, bits):
        ...
        self.tablebin.append(
            [
                self.bitlabel(91, 93, deduct_offset),
                self.vesselID,
                "Vessel ID Type",
        ...
        if self.vesselID == "000":
            ...
            if Func.checkzeros(bits[3:47]):
                ...
                self.tablebin.append(
                    [
                        self.bitlabel(94, 137, deduct_offset),
                        bits[3:47],
        ...
        ###########################
            ...
            if self.mmsi == 111111:
                self.tablebin.append(
                    [
                        self.bitlabel(94, 123, deduct_offset),
                        bits[3:33],
                        "MMSI:",
                        "No MMSI available",
                    ]
            ...
            if self.epirb_ais == 10922:
                self.tablebin.append(
                    [
                        self.bitlabel(124, 137, deduct_offset),
                        bits[33:47],
        ...
        #############################
            ...
            self.tablebin.append(
                [
                    self.bitlabel(94, 135, deduct_offset),
                    bits[3:45],
                    "Radio Callsign",
                    self.callsign,
            ...
            self.tablebin.append(
                [
                    self.bitlabel(136, 137, deduct_offset),
                    bits[45:47],
                    "Spare should be 0",
                    status_check,
                ]
        ...
        ##############################################
            ...
            self.tablebin.append(
                [
                    self.bitlabel(94, 117, deduct_offset),
                    bits[3:27],
            ...
            self.tablebin.append(
                [
                    self.bitlabel(118, 137, deduct_offset),
                    bits[27:47],
                    "Spare should be 0",
                    status_check,
                ]
        ...
        #################################################
            ...
            self.tablebin.append(
                [
                    self.bitlabel(94, 111, deduct_offset),
                    bits[3:21],
            ...
            self.tablebin.append(
                [
                    self.bitlabel(112, 123, deduct_offset),
                    bits[21:33],
            ...
            self.tablebin.append(
                [
                    ...
                    bits[3:47],
                    "Reserved for system testing",
                    "OK. Test protocol bit 43 set to 1. May contain information; default  bits 94-137 - 0s",
                ]
            )
    ...
    def bchmatch(self):
        if len(self.bits) >= 249:
            if (
                Func.errors(Func.calcBCH(self.bits[1:], 0, 202, 250), self.bits[203:])
                > 0
            ):
    ...
    def has_loc(self):
        if (
            self.latitude == "No data available"
            or self.latitude == "Invalid Latitude"
            or self.longitude == "No longitude data available"
            or self.longitude == "Invalid Longitude"
            or self.latitude == "No latitude data available"
        ):
            return False
        else:
            return True
