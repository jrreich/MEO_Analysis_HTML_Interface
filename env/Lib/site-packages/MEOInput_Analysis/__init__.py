...
def find_packets(servername= 'localhost', databasename='MccTestLGM', beaconid = '%', start_date = 0, end_date = None, MEOLUT = '%',ant_i = '%', sat = '%', **kwargs):
    if 'sql_login' in kwargs:
        conn = pypyodbc.connect(r'Driver={FreeTDS};Server='+servername+';Database='+databasename+'; UID='+UID+'; PWD=' + PWD, readonly=True, autocommit=True)
    else:
        conn = pypyodbc.connect(r'Driver={SQL Server}; Server='+servername+'; Database='+databasename+'; Trusted_Connection=yes',readonly=True, autocommit=True)
	c = conn.cursor()
...
def MSSQL_analysis(result, MEOLUTList, TimeStart, TimeEnd, OUTPUTFOLDER, approot, servername = 'localhost', databasename='mccoperational', Lat_GT=0, Long_GT=0, Location='', **kwargs):
    if 'sql_login' in kwargs:
        conn = pypyodbc.connect(r'Driver={FreeTDS};Server='+servername+';Database='+databasename+'; UID='+UID+'; PWD=' + PWD, autocommit=True)
    else:
        conn = pypyodbc.connect(r'Driver={SQL Server};Server='+servername+';Database='+databasename+';Trusted_Connection=yes;',readonly=True, autocommit=True)
...
def MeoDataCollection(beaconId, MEOLUTList, StartTime, EndTime, config_dict, zip_file = False , **kwargs):
    ...
    sql_query = """\
        ...
        """
    params = [StartTime, EndTime, beacon_search_string]
    c = conn.cursor()
    c.execute(sql_query, params)
    ...
    sql_query = """\
        ...
        MeolutId in (%s) 
        """ % placeholders
    params = [beacon_search_string, StartTime, EndTime]
    ...
    if zip_file:
        ...
        outfilelist[J1_file_zip] = 'J1 - Raw Bursts from zip file'
...
def api_meo_location_accuracy(MEOLUT, StartTime, EndTime, config_dict, **kwargs):
    ...
    if out: 
        ...
        for ind, col in enumerate(columns):
            output[col] = 0
            #output['count'] = 0
            output['percent_less_than_5km'] = 0
    ...
    columns = [column[0] for column in c.description]
    out = c.fetchone()
    output['percent_less_than_5km'] = out[0]/float(output['num_of_locations'])
    return output
...
def api_meo_packet_throughput(MEOLUT, StartTime, EndTime, config_dict, rep_rate, minutes, **kwargs):
    ...
    for j in out: 
        ...
        if rep_rate:
            output['antenna'][j[0]]['percent'] = float(j[1])*float(rep_rate)/60/float(minutes)
    return output
def api_site_sum_query(Sitenum, config_dict, **kwargs):
    servername, databasename  = config_dict["servername"], config_dict["oppsdatabase"]
    if 'sql_login' in kwargs:
        conn = pypyodbc.connect(r'Driver={FreeTDS};Server='+servername+';Database='+databasename+'; UID='+UID+'; PWD=' + PWD, readonly=True, autocommit = True)
    else:
...
def api_site(data, config_dict, sitenum = None, **kwargs):
    servername, databasename  = config_dict["servername"], config_dict["oppsdatabase"]
    if 'sql_login' in kwargs:
        conn = pypyodbc.connect(r'Driver={FreeTDS};Server='+servername+';Database='+databasename+'; UID='+UID+'; PWD=' + PWD,readonly=True, autocommit = True)
    else:
