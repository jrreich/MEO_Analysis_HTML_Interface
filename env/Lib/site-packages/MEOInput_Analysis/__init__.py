    ...
    with open(os.path.join(approot,csvoutfile), 'ab') as csvfile:
        ...
        if (filetypesearch == "packet" or filetypesearch == "TOA_FOA_DATA"):   
            for packet in packets:
                ...
                TOA = datetime.strptime(burst[5],datetime_format)
                newrow = [0, burst[4], burst[3], TOA, burst[6], burst[8],TOA,burst[7],burst[9],burst[10],burst[2],burst[0],burst[1]]
...
def MSSQL_analysis(result, MEOLUTList, TimeStart, TimeEnd, OUTPUTFOLDER, approot, servername = 'localhost', databasename='mccoperational', Lat_GT=0, Long_GT=0, Location='', **kwargs):
    if 'sql_login' in kwargs:
        conn = pypyodbc.connect(r'Driver={FreeTDS};Server='+servername+';Database='+databasename+'; UID='+UID+'; PWD=' + PWD)
    else:
        conn = pypyodbc.connect(r'Driver={SQL Server};Server='+servername+';Database='+databasename+';Trusted_Connection=yes;',readonly=True)
...
def MeoDataCollection(beaconId, MEOLUTList, StartTime, EndTime, config_dict, zip_file = False , **kwargs):
    servername, databasename, OUTPUTFOLDER, approot = config_dict["servername"], config_dict["oppsdatabase"], config_dict["OUTPUTFOLDER"], config_dict["approot"]
    if 'sql_login' in kwargs:
        conn = pypyodbc.connect(r'Driver={FreeTDS};Server='+servername+';Database='+databasename+'; UID='+UID+'; PWD=' + PWD)
    else:
    ...
    allsols_file = os.path.join(OUTPUTFOLDER, 'all_sols' + outfiletag +'.csv')
    ...
    country = "USA"
    J1_header = ['BurstId', 'BcnId30','BcnId15','Received','FOA','FOAOff','TOA', 'TOAOff','CN0','BitRate', 'Ant','Sat','MEOLUT']
    print J1_file
    print J2_file
    if zip_file:
        for MEO in MEOLUTList:
            ...
            burstlist = list()
            search_zip_output(MEO, zip_file, zipmatches, filetypesearch, J1_file, approot, J1_header, country, beaconId)


    outfilelist = OrderedDict()
    outfilelist[allsols_file] = 'All Solutions'
    outfilelist[J1_file] = 'J1 - Raw Bursts'
    outfilelist[J2_file] = 'J2 - MEOLUT Solutions'
    outfilelist[J3_file] = 'J3 - MEOLUT Tracked Passes'
    sql_query = """\
        ...
        """
    params = [StartTime, EndTime, beaconId]
    c = conn.cursor()
    c.execute(sql_query, params)
...
#J2 - Format - 
    ...
    sql_query = """\
        ...
        FROM [MccOperationalRpt].[dbo].[PassesTaken] t
		WHERE
...
def api_site_sum_query(Sitenum, servername = 'localhost', databasename='mccoperational', **kwargs):
    if 'sql_login' in kwargs:
        conn = pypyodbc.connect(r'Driver={FreeTDS};Server='+servername+';Database='+databasename+'; UID='+UID+'; PWD=' + PWD)
    else:
        conn = pypyodbc.connect(r'Driver={SQL Server};Server='+servername+';Database='+databasename+';Trusted_Connection=yes;',readonly=True)
    ...
    fieldlist = ['BcnId15', 'BcnId30', 'BcnType', 'CompLat', 'CompLon', 'Closed', 
                 'OpenTime', 'CloseTime', 'TimeFirst', 'TimeLast', 'NumMsgSent', 
                 'MsgTimeLast', 'NumPasses', 'NumMeoSol']
    sql_query_field_list = ', '.join(fieldlist)
...
def api_site(data, sitenum = None, servername = 'localhost', databasename='mccoperational', **kwargs):
    if 'sql_login' in kwargs:
        conn = pypyodbc.connect(r'Driver={FreeTDS};Server='+servername+';Database='+databasename+'; UID='+UID+'; PWD=' + PWD)
    else:
        conn = pypyodbc.connect(r'Driver={SQL Server};Server='+servername+';Database='+databasename+';Trusted_Connection=yes;',readonly=True)
    table = data.get('table','alertsitesum')
    if not sitenum: sitenum = data.get('sitenum',None)
    sql_query = """\
        SELECT CAST( (SELECT * FROM [MccMeoLutMonitor].[dbo].[%s] 
        WHERE AlertSiteNum = ?
        FOR JSON PATH) 
        AS VARCHAR(MAX)) 
        """ % table
...
def api_leo_geo_sols(data, servername = 'localhost', databasename='mccoperational', **kwargs):
    if 'sql_login' in kwargs:
        conn = pypyodbc.connect(r'Driver={FreeTDS};Server='+servername+';Database='+databasename+'; UID='+UID+'; PWD=' + PWD)
    else:
        conn = pypyodbc.connect(r'Driver={SQL Server};Server='+servername+';Database='+databasename+';Trusted_Connection=yes;',readonly=True)
...
def czml_site_meo_input(Sitenum, servername = 'localhost', databasename='mccoperational', **kwargs):
    ...
    for i in out:
        ...
        r[i[0]]= {fieldlist[j]: value for j, value in enumerate(i)}
        #print i[5].isoformat() + 'Z/2017-12-31T:24:00:00z'
...
def czml_meo_orbit_all(starttime, endtime, servername = 'localhost', databasename='mccoperational',  satnum = False, **kwargs):
    if 'sql_login' in kwargs:
        conn = pypyodbc.connect(r'Driver={FreeTDS};Server='+servername+';Database='+databasename+'; UID='+UID+'; PWD=' + PWD)
    else:
        conn = pypyodbc.connect(r'Driver={SQL Server};Server='+servername+';Database='+databasename+';Trusted_Connection=yes;',readonly=True)
    ...
    satdict = OrderedDict()
    sql_query = ('select distinct sat from OrbitVector order by sat ' )
    c = conn.cursor()
    c.execute(sql_query)
    out = c.fetchall()
    if satnum: 
