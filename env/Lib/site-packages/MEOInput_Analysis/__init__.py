...
def MSSQL_beacon_analysis(result, TimeStart, TimeEnd, OUTPUTFOLDER, approot, servername = 'localhost', databasename='mccoperational', gt_file = False, Lat_GT=0, Long_GT=0, Location='', **kwargs):
    ...
    def hist_cum_plot(df,list1,maxrange,title, textstr,legendhandles,textstr2, **args):
        ...
        #nanlist = []
        #print plotlist[0].min(), plotlist[0].max()
        #print plotlist[0]
        #print 'plotlist'
        #print plotlist
        #for ii in range(len(plotlist)):
        #    if np.isnan(plotlist[ii]):
        #        nanlist.append(ii)
        #print 'nanlist'
        #print nanlist
...
def MeoDataCollection(beaconId, MEOLUTList, StartTime, EndTime, OUTPUTFOLDER, approot, servername = 'localhost', databasename='mccoperational', **kwargs):
    if 'sql_login' in kwargs:
        conn = pypyodbc.connect(r'Driver={FreeTDS};Server='+servername+';Database='+databasename+'; UID='+UID+'; PWD=' + PWD)
    else:
        conn = pypyodbc.connect(r'Driver={SQL Server};Server='+servername+';Database='+databasename+';Trusted_Connection=yes;',readonly=True)
    UseMCC = True
    meoListString = "-".join([str(i) for i in MEOLUTList])
    ...
    J3_file = os.path.join(OUTPUTFOLDER, 'J3_' + meoListString + '.csv')
    outfilelist = OrderedDict()
...
#J2 - Format - 
    ...
    sql_query = """\
        ...
        where	(t.LOSStr between ? and ? OR t.AOSStr between ? and ?) 
		        AND t.AntennaId <> 7 
		        AND t.AntennaId <> 8
                AND t.LutId in (%s)
		        order by aos_time
