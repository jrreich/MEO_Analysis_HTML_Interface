...
def MSSQL_beacon_analysis(result, TimeStart, TimeEnd, OUTPUTFOLDER, approot, servername = 'localhost', databasename='mccoperational', gt_file = False, Lat_GT=0, Long_GT=0, Location='', **kwargs):
    ...
    if gt_file:
        ...
        starttime = df_gt.index.min()
        endtime = df_gt.index.max()

        def get_lat_lon_time(df,dateandtime):
            if dateandtime not in df.index:
                newrow = pd.DataFrame(data = [np.nan], index = [dateandtime])
                df = df.append(newrow)
            MXlon = pd.Series(index=df.index, data= df.lon.values).sort_index().interpolate(method='time')[dateandtime]
            MXlat = pd.Series(index=df.index, data= df.lat.values).sort_index().interpolate(method='time')[dateandtime]
            #print MXlon, dateandtime
            #print MXlat, MXlon, dateandtime
            return MXlat, MXlon, dateandtime
    ...
    if (Lat_GT is not 0) and (Long_GT is not 0):
