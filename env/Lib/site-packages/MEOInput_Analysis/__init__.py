...
import pypyodbc
logging.info('pypyodbc ... all imported - look elsewhere')
...
def find_packets(databasename='MccTestLGM', beaconid = '%', start_date = 0, end_date = None, MEOLUT = '%',ant_i = '%', sat = '%', **kwargs):
    if 'sql_login' in kwargs:
        conn = pypyodbc.connect(r'Driver={FreeTDS};Server=reichj-pc\SQLEXPRESS;Database='+databasename+'; UID='+UID+'; PWD=' + PWD)
    else:
...
def MSSQL_analysis(result, MEOLUT, TimeStart, TimeEnd, OUTPUTFOLDER, approot, databasename='mccoperational', Lat_GT=0, Long_GT=0, Location='', **kwargs):
    if 'sql_login' in kwargs:
        conn = pypyodbc.connect(r'Driver={FreeTDS};Server=reichj-pc\SQLEXPRESS;Database='+databasename+'; UID='+UID+'; PWD=' + PWD)
    else:
        conn = pypyodbc.connect(r'Driver={SQL Server};Server=.\SQLEXPRESS;Database='+databasename+';Trusted_Connection=yes;')
...
def MSSQL_burst(result, MEOLUTlist, TimeStart, TimeEnd, OUTPUTFOLDER, approot, databasename='MccTestLGM', **kwargs):
