...
def MeoDataCollection(result, MEOLUTList, StartTime, EndTime, config_dict, zip_file = False , **kwargs):
    ...
    BeaconQuery, beaconId, Lat_GT, Long_GT, Location, MEO_dist = parse_results(result,MEOLUTList)
    if beaconId == '%':
        beacon_out_file_string = 'all'
    else:
        beacon_out_file_string = beaconId
    outfiletag = '_{}_{:%Y-%m-%d-%H%M}_{:%Y-%m-%d-%H%M}'.format(beacon_out_file_string,StartTime,EndTime)
    outfilelist = OrderedDict()
    ...
    if out:
        ...
        outfilelist[allsols_file] = 'All Alert Site Solutions'
...
def api_site(config_dict, sitenum = None, table = None, one= False, **kwargs):
    servername, databasename  = config_dict["servername"], config_dict["oppsdatabase"]
    if 'sql_login' in kwargs:
        conn = pypyodbc.connect(r'Driver={FreeTDS};Server='+servername+';Database='+databasename+'; UID='+UID+'; PWD=' + PWD,readonly=True, autocommit = True)
    else:
