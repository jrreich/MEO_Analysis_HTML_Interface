...
def filter_range(df, field, criteria_low, criteria_high):
    return df[((df[field] > criteria_low) & (df[field] < criteria_high))]

#Write a function to run stats (<5km, 10, 20 , median, quintiles) on a df. 
...
def stats(df,dist_list, q_list):
    ...
    ave = df[errorname].mean()
    median = df[errorname].quantile(.5)
    win_5km = safe_div(df[df[errorname]<5].shape[0],totnum)
    win_10km = safe_div(df[df[errorname]<10].shape[0],totnum)

    q95 = df[errorname].quantile(.95)
    outlist.append(ave)
    for dist in dist_list:
        distout.append(df[df[errorname]<dist].shape[0])
        perout.append(safe_div(df[df[errorname]<dist].shape[0],totnum))
...
#Show a histogram and cumulative plot. Will need to save it and return filename
def hist_cum_plot(df,maxrange,textstr):
    ...
    ar = sorted(df2,reverse=True)
    cumulative = np.cumsum(count).astype(np.float64)
    x = np.linspace(0,bin_range[-1],cumulative.size)
    cumulative/=cumulative.max()
    cumulative*=100.

    ax1 = df2.hist(bins=division)
    ax1.set_ylabel('Number of Locations Produced')
    ax2 = ax1.twinx()
    ax2.set_ylabel('Cumulative Error (%)')
    props = dict(boxstyle='round', facecolor='wheat', alpha=0.5)
    ax2.text(bin_range.mean(), 25, textstr, fontsize=14,
        verticalalignment='top', bbox = props)
    plt.plot(x,cumulative,'ro-')
    #print dfout2.dtypes
    #dfout2.plot.scatter(x=errorname, y=numburstsname, s=dfout2[numsatsname]*5 )
    #dfcumsum = dfout2.cumsum(dfout2[errorname])
    #dfcumsum.plot()
    plt.show()
...
def parse_results(result):
    ...
    print 'test2'
    return BeaconQuery, BeaconID, Lat_GT, Long_GT, Location, MEO_dist, MEOLUT
...
def MSSQL_analysis(result, TimeStart, TimeEnd, OUTPUTFOLDER, approot, servername = 'localhost', databasename='mccoperational', Lat_GT=0, Long_GT=0, Location='', **kwargs):
    ...
    BeaconQuery, BeaconID, Lat_GT, Long_GT, Location, MEO_dist, MEOLUT = parse_results(result)
    ...
    if 'KMLgen' in result:
        ...
        if Lat_GT <> 0:
            ...
            pntEnc.style.iconstyle.icon.href = 'https://maps.google.com/mapfiles/kml/pushpin/ylw-pushpin.png'
            pntEnc.style.iconstyle.scale = 0.8
            pntEnc.style.labelstyle.color = '00ff0000'  # Red
        if 'SingleBurstGen' in result:
            ...
            for row in insols:  
                        if (row[16] <> 'NULL') or row[18]:
                            if row[16] <> 'NULL':
                                ...
                                pnt_RCC.style.iconstyle.icon.href = 'http://maps.google.com/mapfiles/kml/pal4/icon19.png'
                                ...
                                pnt_RCC.style.iconstyle.icon.href = 'http://maps.google.com/mapfiles/kml/pal4/icon25.png'
            ...
            for row in outsols:
                if ((row[17] is not None) and (row[17] <> 'NULL')) or ((row[19] is not None) and (row[19] <> 'NULL')):
                    ...
                    if row[5] == 'MEOFirstAlertDOA':
                        pnt_RCC.style.iconstyle.icon.href = 'http://maps.google.com/mapfiles/kml/pal3/icon0.png'
                    elif row[5] == '406BlownCompDoppler':
                        pnt_RCC.style.iconstyle.icon.href = 'http://maps.google.com/mapfiles/kml/pal3/icon39.png'
                    elif row[5] == 'MEOBlownCompDOA':
                        pnt_RCC.style.iconstyle.icon.href = 'http://maps.google.com/mapfiles/kml/pal3/icon39.png'
                    elif row[5] == 'MEONocrDOA':
                        pnt_RCC.style.iconstyle.icon.href = 'http://maps.google.com/mapfiles/kml/pal3/icon27.png'
                    elif row[5] == 'MEOUpdatedNoCompDOA':
                        pnt_RCC.style.iconstyle.icon.href = 'http://maps.google.com/mapfiles/kml/pal3/icon57.png'
                    elif row[5] == 'MEOUpdatedCompDOA':
                        pnt_RCC.style.iconstyle.icon.href = 'http://maps.google.com/mapfiles/kml/pal5/icon36.png'
                    elif row[5] == '406UpdatedCompDoppler':
                        pnt_RCC.style.iconstyle.icon.href = 'http://maps.google.com/mapfiles/kml/pal5/icon35.png'
                    else:
                        pnt_RCC.style.iconstyle.icon.href = 'http://maps.google.com/mapfiles/kml/paddle/ylw-blank.png'
...
def both_kml(sitenum, OUTPUTFOLDER, approot, servername = 'localhost', databasename = 'MccTestLGM', **kwargs):
    ...
    for row in insols:  
                if ((row[16] is not None) and (row[16] <> 'NULL')) or ((row[18] is not None) and (row[18] <> 'NULL')):
                    if ((row[16] is not None) and (row[16] <> 'NULL')):
                        ...
                        pnt_MEO.style.iconstyle.icon.href = 'http://maps.google.com/mapfiles/kml/pal4/icon19.png'
                        ...
                        pnt_MEO.style.iconstyle.icon.href = 'http://maps.google.com/mapfiles/kml/pal4/icon25.png'
    ...
    kml.save(os.path.join(approot,KMLfile))
    return KMLfile
def MSSQL_beacon_analysis(result, TimeStart, TimeEnd, OUTPUTFOLDER, approot, servername = 'localhost', databasename='mccoperational', Lat_GT=0, Long_GT=0, Location='', **kwargs):
    if 'sql_login' in kwargs:
        conn = pypyodbc.connect(r'Driver={FreeTDS};Server='+servername+';Database='+databasename+'; UID='+UID+'; PWD=' + PWD)
    else:
        conn = pypyodbc.connect(r'Driver={SQL Server};Server='+servername+';Database='+databasename+';Trusted_Connection=yes;',readonly=True)
    ...
    def hist_cum_plot(df,list1,maxrange,title, textstr,legendhandles,**args):
        bin_range = np.arange(0, maxrange+2)
        ...
        ax2.set_ylabel('Cumulative Error (%)')
        props = dict(boxstyle='round', facecolor='wheat', alpha=0.5)
        ax2.text(bin_range.mean(), 25, textstr, fontsize=10,
    ...
    if result.get('filter1check',False):
        if result.get('filter1sel') == 'filter1range':
    ...
    if result.get('filter2check',False):
        if result.get('filter2sel') == 'filter2range':
    ...
    if result.get('filter3check',False):
        if result.get('filter3sel') == 'filter3range':
    ...
    ### still name to add column for location error here

    # Need to write stats to file 
    # if results.get('StatsOut',False):
    ...
    if result.get('histcum',False):
        ...
        filter1crit = numsatsname
        title = 'Varying ' + filter1crit
        ...
        # range needs to be chosen correct for what field is varying - currently good for num sats 
        for i in range(3,10):
            dflist.append(filter1(df,filter1crit,i))
            legendhandles.append(filter1crit + '= ' + str(i))
        
        plt.figure(figsize=(8, 6), dpi=300, facecolor='w', edgecolor='k')
        plt = hist_cum_plot(df,dflist,50,title, textstr1,legendhandles) 
