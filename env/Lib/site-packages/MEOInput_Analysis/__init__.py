...
def MSSQL_beacon_analysis(result, TimeStart, TimeEnd, OUTPUTFOLDER, approot, servername = 'localhost', databasename='mccoperational', Lat_GT=0, Long_GT=0, Location='', **kwargs):
    ...
    UseMCC = True
    if result.get('UseBeaconID') == 'BeaconInput':
        bcn_or_site = 'Beacon'
    else:
        bcn_or_site = 'Site'
    ...
    if result.get('GTSource') == 'GTLatLon':
        if result.get('beaconLat') <> '':
            df_filter['Lat_GT'],df_filter['Lon_GT'] = float(result.get('beaconLat', 0)), float(result.get('beaconLon', 0))
    ...
    if result.get('histcum',False):
        ...
        if result.get('plotvary'):
            ...
            if varyfield == 'numbursts':
                numburstrangediff = 3
                numburstrange = range(0,30,numburstrangediff) # could be variable 
                for i in numburstrange:
                    if i == numburstrange[-1]:
                        dflist.append(filter_range(df_filter,varyfield,i,100))
                        legendhandles.append(varyfield + ': ' + str(i) + '+ ')
                    else:
    ...
    if result.get('KMLgen'): 
        ...
        with open(os.path.join(approot,Solutionfilename), 'rb') as csvfile:
            ...
            for row in filereader:
                if (row[17] <> 'NULL') and (row[17] is not None):  
                    pntEnc = folEnc.newpoint(
                        name= row[6],
