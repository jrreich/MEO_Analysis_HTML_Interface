...
def zip_file_process(MEOLUTs, CSVOUT_TF, SQLDBout_filename, searchtag, Start, End):
    startdate = datetime.datetime(Start).date()
    print startdate
    datelist = [Start + datetime.timedelta(days=x) for x in range(0,(date2-date1).days+1)]
zip_folder = os.getcwd() + UPLOAD_FOLDER
