...
from my_packages.decodefunctions import is_number, dec2bin
import my_packages.meo_input_analysis as MEOInput_Analysis
import my_packages.site_analysis as SiteAnalysis
...
if Deploy_on == "MCC":
    ...
    oppsdatabase_old = (
        "mccoperationalRpt"  # for deploying on operational MCC data from 2018
    )
...
def rawburst():
    ...
    if not arg_dict.get("MEOLUTList"):
        arg_dict["MEOLUTList"] = ["%"]  ## probably not needed
    if arg_dict.get("inputsource") in ["excelfile", "zipfile", "sqldbfile"]:
        ...
        filelist = MEOInput_Analysis.MSSQL_burst(
            arg_dict, config_dict
        )  # ,sql_login = 'yes') # sql_login uses FreeTDS and sql login rather than windows auth - used for linux
        return render_template(
            "BurstAnalysisForm1.html",
            filelist=filelist,
            MEOLUTList=arg_dict["MEOLUTList"],
...
def MEOInputAnalysis():
    ...
    MEOLUTList, StartTime, EndTime = (
        arg_dict["MEOLUTList"],
        arg_dict["StartTime"],
        arg_dict["EndTime"],
    )
    if not result.get("inputsource"):
        result["inputsource"] = "mccdb"
    """
    ...
    if TimeLog:
        print("Starting MSSQL_analysis - " + str(datetime.datetime.utcnow()))
    csvoutfile, filelist = MEOInput_Analysis.MSSQL_analysis(
        result, MEOLUTList, StartTime, EndTime, config_dict
    )
    if TimeLog:
        print("Finished MSSQL_analysis - " + str(datetime.datetime.utcnow()))
    if csvoutfile == None:
        return render_template(
            "MEOInputAnalysisForm1.html",
            NODATA=True,
            result=result,
            StartTime=StartTime,
            EndTime=EndTime,
    ...
    return render_template(
        "MEOInputAnalysisForm1.html", data=csv_data, linklist=filelist
    )
...
def MEOBeaconAnalysis():
    ...
    if result.get("inputsource") == "mcc_operational_rpt":
        config_dict["oppsdatabase"] = oppsdatabase_old
    MEOLUTList, StartTime, EndTime, filesaved = (
        arg_dict["MEOLUTList"],
        arg_dict["StartTime"],
        arg_dict["EndTime"],
        arg_dict.get("filesaved"),
    )
    fileout_dict = {}
    filelist1_dict = None
    # Calling appropriate functions
    if TimeLog:
        print("Starting Analysis calls - " + str(datetime.datetime.utcnow()))
    if result.get("Jdata", False) or result.get("AllSiteSols", False):
        if TimeLog:
            print("  Starting MeoDataCollection - " + str(datetime.datetime.utcnow()))
        filelist1_dict, beacon_out = MEOInput_Analysis.MeoDataCollection(
            result, MEOLUTList, StartTime, EndTime, config_dict
        )
    if TimeLog:
        print("  Starting MSSQL_beacon_analysis - " + str(datetime.datetime.utcnow()))
    (
        beacon_out,
        csvoutfile,
        imglist,
        filelist_dict,
    ) = MEOInput_Analysis.MSSQL_beacon_analysis(
        result, MEOLUTList, StartTime, EndTime, config_dict, filesaved
    )
    if TimeLog:
        print("  Done MSSQL_beacon_analysis - " + str(datetime.datetime.utcnow()))
    ...
    if filelist1_dict is not None:
        fileout_dict.update(filelist1_dict)
    if filelist_dict is not None:
        fileout_dict.update(filelist_dict)
    if csvoutfile is not None:
        fileout_dict.update({csvoutfile: "Summary Stats"})
    output_data = {
        "StartTime": StartTime,
        "EndTime": EndTime,
        "beaconID": beacon_out,
    ...
    return render_template(
        "MEOBeaconAnalysisForm1.html", data=sum_data, output_data=output_data
    )
...
def realtimemonitor():
    ...
    alarmlist, closedalarms, numalarms = MEOInput_Analysis.MEOLUT_alarms(
        StartTime, EndTime, servername, oppsdatabase
    )  # , sql_login = 'yes')
    if TimeLog:
        print("1 - MEO Alarms - " + str(datetime.datetime.utcnow() - startofscript))
    statusHI, statusFL = MEOInput_Analysis.MEOLUT_status(
        StartTime, EndTime, servername, oppsdatabase
    )  # , sql_login = 'yes')
    if TimeLog:
        print("2 - MEO Status - " + str(datetime.datetime.utcnow() - startofscript))
    HI_location_accuracy = MEOInput_Analysis.api_meo_location_accuracy(
        3385, BurstStartTime, EndTime, config_dict
    )[3385]
    ...
    if TimeLog:
        print(
            "3 - HI loc accuracy - " + str(datetime.datetime.utcnow() - startofscript)
        )
    if use_realtimemonitor_sql_db:
        ...
        for i in range(1, 9):
            ...
            HI_packet_percent["antenna"][i]["count"] = int(
                HI_packet_percent["ant" + str(i) + "burstcount"]
            )
            HI_packet_percent["antenna"][i]["percent"] = float(
                HI_packet_percent["ant" + str(i) + "burstpercent"]
            )
    else:
        HI_packet_percent = MEOInput_Analysis.api_meo_packet_throughput(
            3385,
            BurstStartTime,
            EndTime,
            config_dict,
    ...
    if TimeLog:
        print(
            "4 - HI packet percent - " + str(datetime.datetime.utcnow() - startofscript)
        )
    FL_location_accuracy = MEOInput_Analysis.api_meo_location_accuracy(
        3669, BurstStartTime, EndTime, config_dict
    )[3669]
    # FL_location_accuracy = json.loads(get(url_for('meolut_location_accuracy', MEOLUT_ID = 3669, _external = True,
    #                                              StartTime = BurstStartTime_str, EndTime = EndTime_str), verify = False).content)
    if TimeLog:
        print(
            "5 - FL loc accuracy - " + str(datetime.datetime.utcnow() - startofscript)
        )
    if use_realtimemonitor_sql_db:
        ...
        for i in range(1, 9):
            ...
            FL_packet_percent["antenna"][i]["count"] = int(
                FL_packet_percent["ant" + str(i) + "burstcount"]
            )
            FL_packet_percent["antenna"][i]["percent"] = float(
                FL_packet_percent["ant" + str(i) + "burstpercent"]
            )

    else:
        FL_packet_percent = MEOInput_Analysis.api_meo_packet_throughput(
            3669,
            BurstStartTime,
            EndTime,
            config_dict,
    ...
    if TimeLog:
        print(
            "6 - FL packet percent - (total load time) = "
            + str(datetime.datetime.utcnow() - startofscript)
        )
    # open_site_list = MEOInput_Analysis.Open_Sites(servername,oppsdatabase)

    open_site_list = requests.get(
        urlbase + "api/sitesum", verify=False, params={"open_closed": "all_open"}
    ).json()
    num_sites = len(open_site_list) - 1
    return render_template(
        "RealTimeMonitor1.html",
        alarmlist=alarmlist,
    ...
    if request.method == "GET":
        if request.args.get("sitenum") is not None:
            ...
            if request.args.get("Both_KML") is not None:
                KMLfile, Mapfile = MEOInput_Analysis.both_kml(
                    sitenum, OUTPUTFOLDER, approot, servername, oppsdatabase
                )

            alertsitesum = MEOInput_Analysis.alertsitesum_query(
                sitenum, OUTPUTFOLDER, approot, servername, oppsdatabase
            )
            alertsitesols, Input_KMLfile = MEOInput_Analysis.alertsitesol_query(
                sitenum,
                OUTPUTFOLDER,
                approot,
                servername,
            ...
            outsitesols, RCC_KMLfile = MEOInput_Analysis.outsol_query(
                sitenum,
                OUTPUTFOLDER,
                approot,
                servername,
            ...
            open_site_list = MEOInput_Analysis.Open_Sites(
                servername, oppsdatabase
            )  # list of tuples
            open_site_list = requests.get(
                urlbase + "api/sitesum",
                verify=False,
                params={"open_closed": "all_open"},
            ).json()
            all_site_list = requests.get(urlbase + "api/sitesum", verify=False).json()
            no_all_sites = False
            if all_site_list == "None":
                no_all_sites = True
            return render_template(
                "OpenSites1.html",
                open_site_list=open_site_list,
...
def meo_schedule():
    ...
    urlbase = url_for("index", _external=True)
    out_request = requests.get(
        urlbase + "api/MEO/schedule",
        params={
            "StartTime": arg_dict["StartTime"],
            "EndTime": arg_dict["EndTime"],
    ...
    return render_template(
        "MEOSchedule.html", arg_dict=arg_dict, json_data=out_request.json()
    )
...
def meo_cf():
    ...
    return render_template(
        "MEO_CrossFilter2.html",
        json_data_url=json_data.url,
...
def site_analysis():
    result = request.args
    if len(result) == 0:
        return render_template("SiteAnalysisForm.html")
    ...
    if TimeLog:
        print("Starting Data Collection - " + str(datetime.datetime.utcnow()))
    data_source_dict = {}
    for ds in data_sources:
        data_source_dict[ds] = json.loads(
            api_site_sols(ds, site_num).get_data().decode("utf-8")
        )
...
def lmdb():
    ...
    if out_request:
        return render_template(
            "LMDB.html", arg_dict=arg_dict, json_data=out_request.json()
        )
...
def sitereturn():
    ...
    if not request.args:
        input_dict.update({"days": 5})
    arg_dict = url_arg_processor(input_dict)
    if request.args.get("open_closed", False):
        arg_dict["open_closed"] = request.args.get("open_closed")
    else:
        arg_dict["open_closed"] = "all"
    if request.args.get("sitenum", False):
        arg_dict["sitenum"] = request.args.get("sitenum")
    arg_dict.pop("MEOLUTList", None)
    arg_dict.pop("minutes", None)
...
def url_arg_processor(url_args, default_hours=24):
    ...
    if url_args.get("RealPastTime") == "RT_yes":
        ...
        if url_args.get("EndTime", False):
            if url_args.get("EndTime").find("T") != -1:
                EndTime = datetime.datetime.strptime(
                    url_args["EndTime"], "%Y-%m-%dT%H:%M"
                )
            else:
                EndTime = datetime.datetime.strptime(
                    url_args["EndTime"], "%Y-%m-%d %H:%M:%S"
                )
    ...
    # Determine StartTime
    if url_args.get("realtimehours"):
        hours = float(url_args.get("realtimehours"))
    else:
        hours = 0
    minutes = hours * 60
    if url_args.get("days"):
        days = float(url_args.get("days"))
    else:
        days = 0
    minutes += days * 24 * 60
    ...
    if url_args.get("StartTime"):
        if url_args.get("StartTime").find("T") != -1:
            StartTime = datetime.datetime.strptime(
                url_args["StartTime"], "%Y-%m-%dT%H:%M"
            )
        else:
            StartTime = datetime.datetime.strptime(
                url_args["StartTime"], "%Y-%m-%d %H:%M:%S"
            )
...
def meolut_location_accuracy(MEOLUT_ID):
    ...
    output = MEOInput_Analysis.api_meo_location_accuracy(
        MEOLUT_ID,
        arg_dict["StartTime"],
        arg_dict["EndTime"],
        config_dict,
        beaconId=arg_dict["beaconId"],
    )
...
def api_meo_location_accuracy_all():
    ...
    arg_dict["MEOLUTList"] = [
        x for x in request.args.getlist("MEOLUT")
    ]  # defaults to emptylist (all if not specified)
    outdata = MEOInput_Analysis.api_meo_accuracy_all(
        config_dict, arg_dict, request.args.get("output_format", None)
    )
    if outdata:
        if arg_dict["output_format"] == "csv":
            return outdata
        return jsonify(outdata)
    return jsonify("None")
...
def api_meolut_referenece_location_accuracy(MEOLUT_ID):
    ...
    # StartTime = datetime.datetime(2019,01,21,01,00)
    # EndTime = datetime.datetime(2019,01,21,02,00)
    output = MEOInput_Analysis.api_meo_ref_beacon_accuracy(
        MEOLUT_ID, arg_dict["StartTime"], arg_dict["EndTime"], arg_dict, config_dict
    )
    return jsonify(output)
...
def api_meolut_referenece_beacon_locations():
    ...
    kwargs = {}
    arg_dict = url_arg_processor(request.args)
    if not request.args.get("MEOLUT", False):
        arg_dict.pop("MEOLUTList", False)
...
def meolut_packet_throughput(MEOLUT_ID):
    ...
    kwargs = {}
    arg_dict = url_arg_processor(request.args)
    packets = MEOInput_Analysis.api_meo_packet_throughput(
        MEOLUT_ID,
        arg_dict["StartTime"],
        arg_dict["EndTime"],
        config_dict,
        rep_rate=arg_dict["rep_rate"],
...
def api_meo_sched():
    """/api/MEO/schedule?MEOLUT=3385&StartTime=2019-01-21 01:00:00&EndTime=2019-01-21 02:00:00"""
    arg_dict = url_arg_processor(request.args)
    outdata = MEOInput_Analysis.api_meo_schedule(
        arg_dict["MEOLUTList"],
        arg_dict["StartTime"],
        arg_dict["EndTime"],
...
def api_leo_lmdb():
    ...
    arg_dict["LEOLUTList"] = [
        x for x in request.args.getlist("LEOLUT")
    ]  # defaults to emptylist (all if not specified)
    arg_dict["SATList"] = [
        int(x) for x in request.args.getlist("SAT")
    ]  # defaults to emptylist (all if not specified)
    outdata = MEOInput_Analysis.api_leo_lmdb(
        config_dict, arg_dict, request.args.get("output_format", None)
    )
    if outdata:
        if arg_dict["output_format"] == "csv":
...
def api_site(table, sitenum):
    ...
    if table not in [
        ...
        raise TypeError(
            "table must be type 'alertsitesol', 'alertsitesum', 'outsolution', 'leo','meo' or 'enc' not {}".format(
                table
            )
        )
...
def api_site_sols(type, sitenum):
    ...
    if type[:3] == "out":
        url = urlbase + "api/site/outsolution/{}".format(sitenum)
    composite_columns = [
        "alertsitenum",
        "alertsitesolid",
        "gentime",
    ...
    columns_dict = {
        "comp": composite_columns,
        "leo": leo_columns,
        "enc": enc_columns,
        "meo": meo_columns,
        "outcomp": composite_columns,
        "outleo": leo_columns,
        "outenc": enc_columns,
        "outmeo": meo_columns,
    }
    loc_dict = {
        "comp": ("complat", "complon"),
        "leo": ("a_lat", "a_lon"),
        "enc": ("enclat", "enclon"),
        "meo": ("latitude", "longitude"),
        "outcomp": ("complat", "complon"),
        "outleo": ("a_lat", "a_lon"),
        "outenc": ("enclat", "enclon"),
        "outmeo": ("latitude", "longitude"),
    }
    ...
    if type == "all" or type == "output":
        ...
        for row in data:
            if row[field_to_check] != "null" and row[field_to_check] is not None:
                ...
                row_dict.update(
                    {"lat": row[loc_dict[type][0]], "lon": row[loc_dict[type][1]]}
                )
...
def czml_meo_sat_orbit_date(input_date):
    ...
    OrbitData = MEOInput_Analysis.czml_meo_orbit_all(
        starttime, endtime, config_dict
    )  # , satnum)
...
def czml_meolut_ant_per(sourceId):
    ...
    if sourceId == 3385:
        beaconId = "AA5FC0000000001"
    packet_percent = MEOInput_Analysis.api_meo_packet_throughput(
        sourceId,
        StartTime,
        EndTime,
        config_dict,
    ...
    meo_czml = MEOInput_Analysis.czml_meo_ant_per(
        packet_percent,
        StartTime,
        EndTime,
    )
...
def czml_leo_sat_orbit():
    ...
    OrbitData = MEOInput_Analysis.czml_leo_orbit_all(
        starttime, endtime, servername, oppsdatabase
    )  # , satnum)
    return jsonify(OrbitData)
...
def czml_meo_schedule_all():
    ...
    # endtime = datetime.datetime.utcnow() + datetime.timedelta(days = 1)
    # starttime = endtime - datetime.timedelta(days = 1)
    MeoSchedData = MEOInput_Analysis.czml_meo_sched_all(
        starttime, endtime, servername, oppsdatabase
    )  # , antenna)
...
def messager():
    if request.method == "GET":
        ...
        return render_template(
            "messagesent.html", phonenumber=phonenumber, messagetext=messagetext
        )
    else:
        return "<h2> Invalid Request </h2>"
